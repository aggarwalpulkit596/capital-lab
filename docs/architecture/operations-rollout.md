...
The [interactive dashboard](../dashboard.md) now runs 20 curated scenarios across eligibility, illustrative monitoring, reservations, bank recovery, payment matching, and bounded final collection allocation. The full local suite has 72 passing tests. The production acceptance matrix above still includes unimplemented paths, including returns, partial/aggregated collections, and residual payouts. The Apache 2.0 project includes an attributed public-data replay and a CI workflow; it has not been published or deployed publicly.
