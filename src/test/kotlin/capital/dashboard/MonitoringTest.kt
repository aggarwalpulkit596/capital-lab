...
class MonitoringTest {
    @Test
    fun `cancellation threshold includes equality and cannot overflow`() {
        assertTrue(monitor(RiskInputs(100_000, 9_999, 100_000, 7)).isEmpty())
        assertEquals(
            "HIGH_CANCELLATIONS",
            monitor(RiskInputs(100_000, 10_000, 100_000, 7)).single().code,
        )
        assertTrue(
            monitor(RiskInputs(Long.MAX_VALUE, Long.MAX_VALUE, Long.MAX_VALUE, 7)).any {
                it.code == "HIGH_CANCELLATIONS"
            }
        )
    }

    @Test
    fun `spike needs seven preceding observations and positive baseline`() {
    ...
    @Test
    fun `public aggregate has provenance and preserves source arithmetic`() {
        ...
        for (day in PublicDataset.days) assertEquals(
            day["grossMinor"].asLong - day["cancellationsMinor"].asLong,
            day["netMinor"].asLong,
        )
        assertEquals(
            539_392,
            PublicDataset.days.sumOf { it["saleLines"].asInt + it["cancellationLines"].asInt },
        )
        assertEquals(
            PublicDataset.days.map { it["date"].asString }.sorted(),
            PublicDataset.days.map { it["date"].asString },
        )
    }

    @Test
    fun `replay has explicit FX rounding and no future observations in baseline`() {
        ...
        assertEquals(
            PublicDataset.toDemoUsd(5_896_079),
            PublicDataset.inputs("2010-12-02").baselineGrossCents,
        )
    ...
    @Test
    fun `reconciliation requires identity currency amount and posting evidence`() {
        val payment = PaymentEvidence("key", "bank-1", 19_500, "USD", "SETTLED", 19_500)
        val line = StatementLine("key", "bank-1", 19_500, "USD")
        fun status(p: PaymentEvidence = payment, s: List<StatementLine> = listOf(line)) =
            reconcile(listOf(p), s).first().status
    ...
    @Test
    fun `unknown bank debits and unsettled local operations remain exceptions`() {
        val line = StatementLine("key", "bank-1", 19_500, "USD")
        ...
        assertEquals(
            "UNPOSTED_BANK_DEBIT",
            reconcile(listOf(payment), listOf(line)).single().status,
        )
        assertEquals("PENDING", reconcile(listOf(payment), emptyList()).single().status)
        assertEquals(
            "NO_CASH_EXPECTED",
            reconcile(listOf(payment.copy(state = "REJECTED")), emptyList()).single().status,
        )
    }
}
