...
fun main() {
    ...
    val database = Database(env("LAB_JDBC_URL"), env("LAB_DB_USER"), env("LAB_DB_PASSWORD"), env("LAB_SCHEMA"))
    val http = HttpBankGateway(URI(env("LAB_BANK_URI")))
    val bank = object : BankGateway {
    ...
    AdvanceService(database, clock, LocalDate.parse("2026-09-18")).process(UUID.fromString(env("LAB_ADVANCE_ID")), bank)
