...
class ReservationIntegrationTest {
    private val postgres = PostgreSQLContainer("postgres:17-alpine")
        ...
        .withCreateContainerCmdModifier { command ->
            command.hostConfig!!.withPortBindings(PortBinding(Ports.Binding.bindIpAndPort("127.0.0.1", 0), ExposedPort(5432)))
        }
    ...
    @BeforeEach fun setup() {
        ...
        database = Database(postgres.jdbcUrl, postgres.username, postgres.password, "capital_$suffix")
        bankDatabase = Database(postgres.jdbcUrl, postgres.username, postgres.password, "bank_$suffix")
    ...
    @AfterEach fun teardown() { if (::server.isInitialized) server.close() }
    ...
    private fun advanceRetry() { clock.advance(Duration.ofSeconds(3)) }
    ...
    private fun <T> concurrently(count: Int, operation: (Int) -> T): List<T> {
        ...
        try {
            val tasks = (0 until count).map { index -> executor.submit<T> { ready.countDown(); check(start.await(5, TimeUnit.SECONDS)); operation(index) } }
    ...
    @Test fun `simultaneous different requests cannot spend the same pool capacity`() {
        val results = concurrently(8) { index -> runCatching { newService().reserve(request, "request-$index") } }
    ...
    @Test fun `simultaneous identical requests return the same persisted operation`() {
    ...
    @Test fun `same request key cannot change principal or destination`() {
        val original = service.reserve(request, "key")
        assertFailsWith<IdempotencyConflict> { service.reserve(request.copy(principalCents = 10_000), "key") }
        assertFailsWith<IdempotencyConflict> { service.reserve(request.copy(destinationVersion = "other"), "key") }
        assertEquals(original.id, service.reserve(request, "key").id)
        assertEquals(19_500, number("SELECT reserved_cash_cents FROM treasury"))
    }

    @Test fun `borrower limit is shared across distinct receivable pools`() {
        database.transaction {
            it.update("UPDATE developers SET limit_cents=80000 WHERE id='developer-1'")
            it.update("""INSERT INTO pools(id,developer_id,net_proceeds_cents,report_through,downloaded_at)
                SELECT 'pool-2',developer_id,100000,report_through,downloaded_at FROM pools WHERE id='pool-1'""")
        }
        val results = concurrently(2) { index -> runCatching { newService().reserve(request.copy(poolId = "pool-${index + 1}", principalCents = 15_000), "pool-request-$index") } }
        assertEquals(1, results.count { it.isSuccess })
        assertEquals(15_000, number("SELECT reserved_cents FROM developers"))
    }

    @Test fun `funding liquidity is shared across distinct developers`() {
        database.transaction {
            it.update("UPDATE treasury SET cash_cents=20000")
            it.update("INSERT INTO developers(id,limit_cents,destination_version) VALUES ('developer-2',200000,'verified-destination-v1')")
            it.update("""INSERT INTO pools(id,developer_id,net_proceeds_cents,report_through,downloaded_at)
                SELECT 'pool-2','developer-2',100000,report_through,downloaded_at FROM pools WHERE id='pool-1'""")
        }
        val results = concurrently(2) { index -> runCatching { newService().reserve(request.copy(developerId = "developer-${index + 1}", poolId = "pool-${index + 1}"), "cash-$index") } }
        assertEquals(1, results.count { it.isSuccess })
        assertEquals(19_500, number("SELECT reserved_cash_cents FROM treasury"))
    }

    @Test fun `lost response preserves capacity and query recovers one executed transfer`() {
        ...
        assertEquals(0, number("SELECT sum(CASE WHEN side='DEBIT' THEN cents ELSE -cents END) FROM ledger_entries"))
        assertFalse(service.process(advance.id, bank))
        assertEquals(1, number("SELECT count(*) FROM journals"))
    }

    @Test fun `hold permits recording an already executed transfer using query only`() {
        val advance = service.reserve(request, "key")
        server.nextFault.set(BankFault.LOSE_RESPONSE_AFTER_COMMIT)
    ...
    @Test fun `hold never resubmits missing transfer and retains both reservations`() {
        val advance = service.reserve(request, "key")
    ...
    @Test fun `missing transfer without hold resubmits the same frozen operation`() {
        val advance = service.reserve(request, "key")
        server.nextFault.set(BankFault.LOSE_RESPONSE_BEFORE_EXECUTION)
        service.processNext(bank)
        advanceRetry()
        ...
        assertEquals(advance.providerKey, bankDatabase.transaction { it.rows("SELECT provider_key FROM bank_operations") { row -> row.getString(1) }.single() })
    }

    @Test fun `hold arriving during recovery lookup stops resubmission`() {
        val advance = service.reserve(request, "key")
        server.nextFault.set(BankFault.LOSE_RESPONSE_BEFORE_EXECUTION)
        service.processNext(bank)
        advanceRetry()
        val racingHold = object : BankGateway {
    ...
    @Test fun `definitive bank rejection releases funds without booking a payment`() {
        val advance = service.reserve(request, "key")
    ...
    @Test fun `new hold cancels a provably undispatched request`() {
        val advance = service.reserve(request, "key")
        service.setHold("developer-1", true)
    ...
    @Test fun `changed destination cancels undispatched frozen payment rather than rerouting`() {
        val advance = service.reserve(request, "key")
        database.transaction { it.update("UPDATE developers SET destination_version='verified-destination-v2'") }
        service.processNext(bank)
        assertEquals("CANCELED", service.get(advance.id).state)
    ...
    @Test fun `stale source and wrong developer create no reservation or outbox command`() {
        ...
        assertFailsWith<FundingDeclined> { service.reserve(request.copy(developerId = "wrong-developer"), "wrong-owner") }
        assertEquals(0, number("SELECT count(*) FROM advances"))
        assertEquals(0, number("SELECT count(*) FROM outbox"))
        assertEquals(0, number("SELECT reserved_cash_cents FROM treasury"))
    }

    @Test fun `partial repayment snapshot cannot recycle lifetime origination capacity`() {
        database.transaction {
            it.update("UPDATE pools SET settled_proceeds_cents=50000,funded_lifetime_cents=80000,outstanding_cents=30000")
            it.update("UPDATE developers SET outstanding_cents=30000")
        }
        assertFailsWith<FundingDeclined> { service.reserve(request.copy(principalCents = 10_000), "refinance") }
        assertEquals(0, number("SELECT count(*) FROM advances"))
    }

    @Test fun `bank idempotency prevents repeated debit and rejects changed payload`() {
        val advance = service.reserve(request, "key")
    ...
    @Test fun `separate JVM crash after bank commit recovers the same transfer`() {
        val advance = service.reserve(request, "crash")
        val java = Path.of(System.getProperty("java.home"), "bin", "java").toString()
        val log = Files.createTempFile("capital-crash-worker", ".log")
        val builder = ProcessBuilder(java, "-cp", checkNotNull(System.getProperty("lab.test.classpath")), "capital.payments.CrashWorkerKt")
        builder.environment().putAll(mapOf(
            "LAB_JDBC_URL" to postgres.jdbcUrl, "LAB_DB_USER" to postgres.username, "LAB_DB_PASSWORD" to postgres.password,
            "LAB_SCHEMA" to database.schema, "LAB_BANK_URI" to server.endpoint.toString(),
            "LAB_ADVANCE_ID" to advance.id.toString(), "LAB_NOW" to clock.instant().toString(),
        ))
    ...
    @Test fun `only one worker holds the active dispatch lease`() {
        val advance = service.reserve(request, "lease")
        val entered = CountDownLatch(1)
        val release = CountDownLatch(1)
        val executor = Executors.newSingleThreadExecutor()
        val pausedBank = object : BankGateway {
            override fun submit(command: BankCommand): BankResult { entered.countDown(); check(release.await(10, TimeUnit.SECONDS)); return bank.submit(command) }
    ...
    @Test fun `expired worker cannot overwrite a newer settled observation`() {
        val advance = service.reserve(request, "fencing")
        val paid = CountDownLatch(1)
        val release = CountDownLatch(1)
        val executor = Executors.newSingleThreadExecutor()
        val slowBank = object : BankGateway {
            override fun submit(command: BankCommand): BankResult {
                assertIs<BankResult.Settled>(bank.submit(command)); paid.countDown()
    ...
    @Test fun `failed local posting rolls back balances and remains recoverable`() {
        ...
        assertEquals(0, number("SELECT count(*) FROM journals"))
        database.transaction { it.createStatement().use { statement -> statement.execute("DROP TRIGGER injected_failure ON journals") } }
        clock.advance(Duration.ofSeconds(31))
        assertTrue(service.processNext(bank))
        assertEquals("SETTLED", service.get(advance.id).state)
        assertEquals(1, server.postRequests.get())
    ...
    @Test fun `database rejects unbalanced journals and edits to posted history or frozen commands`() {
        val advance = service.reserve(request, "ledger")
        assertFailsWith<SQLException> {
            database.transaction { connection ->
                val journal = UUID.randomUUID()
                connection.update("INSERT INTO journals(id,advance_id,posting_key,created_at) VALUES (?,?,?,?)", journal, advance.id, "invalid", clock.instant())
                connection.update("INSERT INTO ledger_entries(journal_id,account,currency,side,cents) VALUES (?,'ADVANCE_RECEIVABLE','USD','DEBIT',20000)", journal)
            }
        }
        assertEquals(0, number("SELECT count(*) FROM journals"))
        service.processNext(bank)
        assertFailsWith<SQLException> { database.transaction { it.update("UPDATE ledger_entries SET cents=1") } }
        assertFailsWith<SQLException> { database.transaction { it.update("UPDATE advances SET destination_version='changed'") } }
        assertFailsWith<SQLException> {
            database.transaction { connection ->
                connection.update("""INSERT INTO ledger_entries(journal_id,account,currency,side,cents)
