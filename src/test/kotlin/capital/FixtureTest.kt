...
class FixtureTest {
    ...
    @Test
    fun `saved fixtures match hand worked expectations`() {
        val expected =
            Files.newBufferedReader(path).use {
                JsonParser.parseReader(it).asJsonObject.getAsJsonArray("cases")
            }
        ...
        fixtures.forEachIndexed { index, fixture ->
            ...
            assertEquals(
                row["expectedArithmeticCapacityCents"].asLong,
                result.arithmeticCapacityCents,
                fixture.name,
            )
            assertEquals(
                row["expectedPrincipalCents"].asLong,
                result.eligiblePrincipalCents,
                fixture.name,
            )
            ...
            assertEquals(
                row["expectedExcessExposureCents"].asLong,
                result.excessExposureCents,
                fixture.name,
            )
        }
    }
    ...
    @Test
    fun `decision JSON preserves inputs and distinguishes held diagnostic capacity`() {
        ...
        assertEquals(
            ...
            json.getAsJsonObject("snapshot")["downloadedAt"].asString,
        )
        assertEquals("illustrative-v1", json.getAsJsonObject("policy")["version"].asString)
        assertEquals(
            "2026-09-18",
            json.getAsJsonObject("context")["minimumReportThroughDate"].asString,
        )
    }

    @Test
    fun `fractional cents cannot be silently truncated at input`() {
        val input =
            Files.readString(path)
                .replace("\"netProceedsCents\": 100000", "\"netProceedsCents\": 100000.5")
    ...
    @Test
    fun `missing amount cannot default to zero`() {
    ...
    @Test
    fun `unsupported currency cannot be silently labelled USD`() {
