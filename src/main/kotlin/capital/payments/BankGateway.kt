...
import capital.config.BankHttpSettings
...
internal fun BankCommand.hash(): String =
    fingerprint(providerKey, amountCents, currency, destination)

/**
 * Only targets the local simulator. A real bank adapter requires its own authentication and
 * contract.
 */
class HttpBankGateway(
    private val endpoint: URI,
    private val settings: BankHttpSettings = BankHttpSettings(),
) : BankGateway {
    init {
        require(endpoint.scheme == "http" && endpoint.host == "127.0.0.1") {
            "Simulator endpoint must be loopback HTTP"
        }
    }

    private val client = HttpClient.newBuilder().connectTimeout(settings.connectTimeout).build()

    override fun submit(command: BankCommand): BankResult {
        val body = Gson().toJson(command)
        val request =
            HttpRequest.newBuilder(endpoint.resolve("/transfers"))
    ...
    override fun lookup(command: BankCommand): BankResult {
        ...
        val request =
            HttpRequest.newBuilder(endpoint.resolve("/transfers/${command.providerKey}"))
                .timeout(settings.responseTimeout)
                .GET()
                .build()
    ...
    private fun send(
        ...
        try {
            // A request/connect timeout alone did not bound a partially delivered response body in
            // testing.
            // Bound the future that includes body consumption, then cancel the transport on any
            // failure.
            val pending = client.sendAsync(request, HttpResponse.BodyHandlers.ofString())
            val response =
                try {
                    pending.get(settings.responseTimeout.toMillis(), TimeUnit.MILLISECONDS)
            ...
            if (allowNotFound && response.statusCode() == 404) BankResult.NotFound
                ...
                BankResult.Unknown("HTTP_${response.statusCode()}")
            else {
                ...
                when (json.requiredString("state")) {
                    "SETTLED" ->
                        BankResult.Settled(
                            json.requiredString("transferId").also { require(it.isNotBlank()) }
                        )
