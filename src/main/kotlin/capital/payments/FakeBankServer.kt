...
enum class BankFault {
    NONE,
    LOSE_RESPONSE_AFTER_COMMIT,
    LOSE_RESPONSE_BEFORE_EXECUTION,
    REJECT_BEFORE_PAYMENT,
}
...
class FakeBankServer(private val database: Database) : AutoCloseable {
    ...
    private val server =
        HttpServer.create(InetSocketAddress("127.0.0.1", 0), 0).also {
            ...
            when (exchange.requestMethod) {
                "GET" -> {
                    ...
                    val record = database.transaction { connection ->
                        connection
                            .rows(
                                "SELECT * FROM bank_operations WHERE provider_key = ?",
                                key,
                                read = ::asJson,
                            )
                            .singleOrNull()
                    }
                    if (record == null) respond(exchange, 404, "{}")
                    else respond(exchange, 200, record.toString())
                }
                "POST" -> {
                    postRequests.incrementAndGet()
                    val body =
                        exchange.requestBody.bufferedReader().use {
                            JsonParser.parseReader(it).asJsonObject
                        }
                    val command =
                        BankCommand(
                            body.requiredString("providerKey"),
                            body["amountCents"].asBigDecimal.longValueExact(),
                            body.requiredString("currency"),
                            body.requiredString("destination"),
                        )
                    require(
                    ...
                    val record = database.transaction { connection ->
                        val balance =
                            connection.number(
                                "SELECT cents FROM bank_balance WHERE id = 1 FOR UPDATE"
                            )
                        val existing =
                            connection
                                .rows(
                                ...
                                .singleOrNull()
                        if (existing != null) {
                            require(existing.first == command.hash()) {
                                "Idempotency payload conflict"
                            }
                            connection.update(
                                "UPDATE bank_operations SET submit_count = submit_count + 1 WHERE provider_key = ?",
                                command.providerKey,
                            )
                            existing.second
                        } else {
                            val state =
                                if (
                                    fault == BankFault.REJECT_BEFORE_PAYMENT ||
                                        balance < command.amountCents
                                )
                                    "REJECTED"
                                else "SETTLED"
                            connection.update(
                                """INSERT INTO bank_operations(provider_key,payload_hash,transfer_id,currency,amount_cents,destination,state)
                                VALUES (?,?,?,?,?,?,?)""",
                                command.providerKey,
                                command.hash(),
                            ...
                            if (state == "SETTLED")
                                connection.update(
                                    "UPDATE bank_balance SET cents = cents - ? WHERE id = 1",
                                    command.amountCents,
                                )
                            connection
                                .rows(
    ...
    private fun asJson(row: ResultSet): JsonObject =
        JsonObject().apply {
