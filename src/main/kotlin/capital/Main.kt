...
data class Fixture(val name: String, val snapshot: Snapshot, val policy: Policy, val context: EvaluationContext)
...
fun loadFixtures(path: Path): List<Fixture> {
    ...
    require(root.string("currency") == "USD") { "This prototype only accepts USD; conversion is not implemented" }
    ...
    return root.getAsJsonArray("cases").map { element ->
        ...
        Fixture(
            item.string("name"),
            Snapshot(
                item.string("poolId"), item.string("snapshotId"),
                LocalDate.parse(item.string("reportThroughDate")), Instant.parse(item.string("downloadedAt")),
                item.cents("netProceedsCents"), item.cents("existingExposureCents"),
            ),
            Policy(root.string("policyVersion"), item.cents("exposureCeilingCents"),
                allowedReportingLagDays = root.cents("allowedReportingLagDays").let { Math.toIntExact(it) }),
            context,
        )
    }
}
...
fun decisionRecord(name: String, decision: Decision): JsonObject = JsonObject().apply {
    ...
    add("context", JsonObject().apply {
        ...
        addProperty("expectedReportThroughDate", decision.context.expectedReportThroughDate.toString())
        addProperty("reportingZone", decision.context.reportingZone.id)
        addProperty("minimumReportThroughDate", decision.minimumReportThroughDate.toString())
    })
...
fun main(args: Array<String>) {
    ...
    fixtures.forEach { fixture -> records.add(decisionRecord(fixture.name, evaluate(fixture.snapshot, fixture.policy, fixture.context))) }
